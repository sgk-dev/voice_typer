...
Simulates the paste shortcut (Ctrl+V, or Ctrl+Shift+V for terminals) through a
...
`wtype` is unavailable and keycode-level typing is layout-dependent, so the
transcribed text is always delivered via clipboard + Ctrl+V (see SgkClipboard).
"""
...
class SgkUinputInjector:
    ...
    def __init__(
        self,
        name: str = "SGK VoiceTyper Virtual Keyboard",
        ui: object | None = "auto",
    ) -> None:
