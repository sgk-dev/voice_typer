"""XDG autostart entry management for VoiceTyper.
...
from sgk_voice_typer.utils.resources import sgk_desktop_entry

_logger = sgk_get_logger(__name__)

SGK_AUTOSTART_PATH = Path.home() / ".config" / "autostart" / "voice-typer.desktop"


def sgk_default_exec_cmd() -> str:
    """Best-effort command that starts the daemon with its tray."""
    launcher = Path.home() / ".local" / "bin" / "sgk-voice-typer"
    if launcher.exists():
        return str(launcher)
    found = shutil.which("sgk-voice-typer")
    if found:
        return found
    return "sgk-voice-typer"
