...
Two running daemons means two evdev listeners, so every push-to-talk press
fires twice and the two recordings race each other. This takes an exclusive
``flock`` on a lock file for the lifetime of the process; a second launch
fails to take it and should exit.
"""
