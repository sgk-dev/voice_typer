...
from sgk_voice_typer.utils import autostart


@pytest.fixture
def autostart_path(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    target = tmp_path / "autostart" / "voice-typer.desktop"
...
def test_enable_creates_valid_desktop_entry(autostart_path: Path) -> None:
    state = autostart.sgk_set_autostart(True, exec_cmd="/usr/bin/sgk-voice-typer")
    assert state is True
    content = autostart_path.read_text(encoding="utf-8")
    assert "[Desktop Entry]" in content
    assert "Exec=/usr/bin/sgk-voice-typer" in content
    assert "Name=VoiceTyper" in content
...
def test_default_exec_cmd_is_a_nonempty_string() -> None:
